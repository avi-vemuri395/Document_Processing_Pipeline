...
class EnhancedExtractionPipeline:
    ...
    async def process_loan_package(
        ...
        logger.info(f"Processing loan package: {application_id}")
        logger.info(f"Documents to process: {len(document_paths)}")
        
        # Phase 1: Classify all documents
        logger.info("Phase 1: Document classification")
        ...
        logger.info("Phase 3: Building loan application")
        ...
        if self.config.enable_cross_validation:
            logger.info("Phase 4: Cross-document validation")
    ...
    async def _classify_documents(
        ...
        for doc_path in document_paths:
            ...
            try:
                ...
                logger.info(f"Classified {doc_path.name} as {result.primary_type.value} "
    ...
    def _extract_text(self, doc_path: Path) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Text extraction failed for {doc_path}: {e}")
            return None
